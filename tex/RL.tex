...
\begin{CJK}{UTF8}{gbsn}
    ...
    ⑤行为策略采集数据，利用数据计算TD-target（数据组织形式）， \\[3pt]
    目标策略的估计值趋向TD-target，即TD-target代表着目标策略。 \\[3pt]
    ⑥on-policy: 用一个策略和环境交互，得到experience， 
    估计这个策略，改进这个策略。 \\[3pt]
    再用改进的策略循环，交互、估计、改进，直至最优（这个系列的最优）。 \\[3pt]
    off-policy: 一个策略和环境交互，得到experience， \\[3pt]
    另一个策略被估计、被改进。（目标策略不易采集数据，所以需要行为策略，异轨） \\[3pt]
    experience应当具有一些性质，能够架通两个策略， \\[3pt]
    行为策略能够采到，目标策略能够使用。 \\[3pt]
\end{CJK}
